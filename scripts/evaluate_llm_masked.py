...
def run_evaluation(input_path, model_name="google/flan-t5-base", batch_size=8, limit=None):
    ...
    print(df[['masked_instruction', 'llm_output_raw']].tail())

if __name__ == "__main__":
    DEGRADATION_PATH = os.path.join(BASE_DIR, "reports", "llm_audits", "LLM_DEGRADATION_INPUT.parquet")
    ...
    if os.path.exists(DEGRADATION_PATH):
        ...
        run_evaluation(DEGRADATION_PATH, batch_size=32, limit=None)
    else:
        print(f"Error: Could not find Degradation Input at {DEGRADATION_PATH}")