...
— OLMo-2-0425-1B-Instruct (4-Shot ICL)

Pipeline per sample:
  original instruction → OLMo-2-0425-1B-Instruct → predicted label
                       → parse to {Answerable, Ambiguous, Contradictory}
                       → compare against oracle_label (mode='label', three-way)

Completes the unmasked baseline evaluation alongside FLAN-T5-base and
FLAN-T5-large original instruction results, closing the model asymmetry
in the masked vs unmasked comparison.

Input:  reports/llm_audits/ORIGINAL_ORACLE_LABELS.parquet
Output: reports/llm_audits/LLM_ANSWERABILITY_RESULTS_ORIGINAL_OLMO.parquet
"""
...
def run_evaluation(input_path: str,
                   ...
                   output_filename: str = "LLM_ANSWERABILITY_RESULTS_ORIGINAL_OLMO.parquet",
                   batch_size: int = 8,
                   limit: int = None):
    ...
    for i in tqdm(range(0, len(df), batch_size)):
        ...
        # Key difference from masked script: we use original_text column
        prompts = [
