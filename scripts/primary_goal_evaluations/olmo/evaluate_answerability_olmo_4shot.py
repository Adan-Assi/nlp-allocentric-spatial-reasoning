"""
Answerability Classification Evaluation — OLMo-2-0425-1B-Instruct
...
Key architectural difference from FLAN-T5 scripts:
  - Uses AutoModelForCausalLM (decoder-only) instead of AutoModelForSeq2SeqLM
  - Decodes only generated tokens, not the prompt
  - Uses left-padding (required for causal LMs in batch inference)

Input:  reports/llm_audits/LLM_DEGRADATION_INPUT.parquet
Output: reports/llm_audits/LLM_ANSWERABILITY_RESULTS_OLMO.parquet

Primary task evaluation. Same prompt and metrics as FLAN-T5 scripts
...
def run_evaluation(input_path: str,
                   ...
                   output_filename: str = "LLM_ANSWERABILITY_RESULTS_OLMO.parquet",
                   batch_size: int = 8,
                   limit: int = None):
...
if __name__ == "__main__":
    ...
    if os.path.exists(DEGRADATION_PATH):
        run_evaluation(
            ...
            output_filename="LLM_ANSWERABILITY_RESULTS_OLMO.parquet",
            batch_size=8,
            limit=None,
        )
    else:
