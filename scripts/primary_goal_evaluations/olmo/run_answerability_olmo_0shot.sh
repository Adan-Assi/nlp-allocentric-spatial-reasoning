#!/bin/bash
#SBATCH --job-name=rvs_olmo_0shot
...
#SBATCH --output=logs/answerability_olmo_0shot_%j.out
#SBATCH --error=logs/answerability_olmo_0shot_%j.err

echo "Starting Answerability Classification — OLMo-2-0425-1B-Instruct (Zero-Shot)"
echo "Date: $(date)"
echo "Node: $(hostname)"
...
python scripts/primary_goal_evaluations/olmo/evaluate_answerability_olmo_0shot.py
