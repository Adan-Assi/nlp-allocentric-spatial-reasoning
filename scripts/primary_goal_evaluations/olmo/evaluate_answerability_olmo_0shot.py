"""
Answerability Classification Evaluation — OLMo-2-0425-1B-Instruct (Zero-Shot)
...
Evaluation uses zero-shot in-context learning:
only the task definition and label descriptions are provided,
with no labeled examples in the prompt.

Input:  reports/llm_audits/LLM_DEGRADATION_INPUT.parquet
Output: reports/llm_audits/LLM_ANSWERABILITY_RESULTS_ZEROSHOT.parquet
"""
...
def run_evaluation(input_path: str,
                   ...
                   output_filename: str = "LLM_ANSWERABILITY_RESULTS_ZEROSHOT.parquet",
                   batch_size: int = 8,
                   limit: int = None):
...
if __name__ == "__main__":
    ...
    if os.path.exists(DEGRADATION_PATH):
        run_evaluation(
            ...
            output_filename="LLM_ANSWERABILITY_RESULTS_ZEROSHOT.parquet",
            batch_size=8,
            limit=None,
        )
    else:
