#!/bin/bash
#SBATCH --job-name=rvs_cls_orig_base
...
#SBATCH --output=logs/answerability_original_base_%j.out
#SBATCH --error=logs/answerability_original_base_%j.err

echo "Starting Answerability Classification on Original Instructions — FLAN-T5-base"
echo "Date: $(date)"
echo "Node: $(hostname)"
...
python scripts/primary_goal_evaluations/evaluate_answerability_original_base.py
